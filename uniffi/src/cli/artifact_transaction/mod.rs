...
pub(in crate::cli) use engine::{
    ...
    persistent_fs_identity, preflight_managed_package, read_owned_tree_marker,
    read_verified_regular_file, read_verified_regular_file_bounded,
    read_verified_regular_file_bounded_with_budget, remove_captured_directory_for_cleanup,
    remove_current_regular_file_for_cleanup_with_budget, remove_owned_tree_for_cleanup,
    require_regular_source_file, sha256_bytes, validate_existing_path_guards,
    validate_inventory_path, validate_owned_tree, write_durable_file, write_owned_tree_marker,
    DirectCommitOutcome, DirectOwnerPlan, GenericPublicationPlan, HspCandidate, HspOutputPaths,
    HspPathGuards, IdentityBoundInvocationRoot, InvocationDist, InvocationOutputSpec,
    InvocationTypeCache, ManagedPackageOwner, ManagedPackageTransaction, ManagedTransactionLayout,
    OutputLock, OwnedTreeEntry, OwnedTreeMarker, OwnedTreeMarkerEntry, OwnedTreeSnapshot,
    PersistentFsIdentity, PreparedHspInvocation, PreparedHspPackage, PublicationHooks,
    SharedTraversalBudget, TraversalBudget, TypeCacheIdentity, TypeCacheInitialization,
    TypeCachePlan, MAX_HSP_ARCHIVE_COMPRESSED_BYTES, MAX_HSP_ARCHIVE_ENTRIES,
    MAX_HSP_ARCHIVE_MEMBER_BYTES, MAX_HSP_ARCHIVE_PATH_BYTES, MAX_HSP_ARCHIVE_TOTAL_BYTES,
    OWNED_TREE_SCHEMA_VERSION, TYPE_CACHE_OWNER_MARKER, TYPE_CACHE_WORK_MARKER,
};
