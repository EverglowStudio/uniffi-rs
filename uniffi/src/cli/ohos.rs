...
const FACADE_CONTRACT_SCHEMA_VERSION: u64 = 3;
  ...
  private finishWithError(reason: Error): void {
    ...
    const error: BusinessError<UniFfiStreamErrorData<E>> =
      reason instanceof UniFfiStreamBusinessError ?
        ...
        new UniFfiStreamBusinessError<E>(
          __UNIFFI_STREAM_SOURCE_ERROR_CODE,
          reason.message.length > 0 ? reason.message : 'UniFFI stream source failed',
          this.errorTypeName(),
          reason.name.length > 0 ? reason.name : 'Error',
          reason.message.length > 0 ? reason.message : 'UniFFI stream source failed',
          null
        );
...
abstract class __UniFfiPullStream<T, E> implements UniFfiStream<T> {
  ...
  private streamError(reason: Error): Error {
    if (reason instanceof UniFfiStreamBusinessError) {
      return reason;
    }
