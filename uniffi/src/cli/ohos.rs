...
fn build_internal(options: BuildOptions) -> Result<Option<PreparedHspInvocation>> {
    ...
    // Validate their contents before invoking Cargo or backend tooling.
    ...
    if options.package_kind == PackageKind::Hsp {
        let result = (|| -> Result<Option<PreparedHspInvocation>> {
            ...
            let mut facade_inputs = Vec::with_capacity(plan.package_count);
                ...
                .enumerate()
            {
                validate_host_facade_bundle_for_package(&facade_bundle, package)?;
                let facade_input =
                    InvocationFacadeInputs::new(&target_dir, package, &facade_bundle)?;
                let type_dir = facade_input.work_dir().to_path_buf();
                let invocation_dist =
                ...
                facade_inputs.push(facade_input);
            }
            // Every package and Hvigor output is fully staged before any public
            // destination changes. Keep invocation facade inputs alive through staging.
            drop(facade_inputs);
        ...
        .zip(&package_dist_paths)
    {
        ...
        let facade_input = InvocationFacadeInputs::new(&target_dir, package, &facade_bundle)?;
        let type_dir = facade_input.work_dir().to_path_buf();
...
fn validate_host_facade_bundle_for_package(
    bundle: &HostFacadeBundle,
    _package: &HostPackage,
) -> Result<()> {
    if bundle.mode == FacadeBundleMode::RawOnly {
        return Ok(());
    }
    if bundle.contracts.is_empty() || bundle.type_sidecars.is_empty() {
        bail!("required OHOS facade bundle must contain contracts and canonical type sidecars");
    }
    Ok(())
}

/// Validate the functional contract and type inputs before any Cargo build.
fn preflight_host_facade_bundle_semantics(
    ...
    let contracts = parse_bundle_contracts(bundle, path)?;
    let owned_defs = parse_owned_bundle_type_defs(bundle, path, &contracts)?;
    let defs = owned_defs
        .iter()
    ...
    let owner_type_names = owned_defs
        .iter()
        .map(|owned| {
            (
    ...
    let owner_native_export_prefixes = owned_defs
        .iter()
        .map(|owned| {
            (
                owned.projection.component.clone(),
...
fn parse_bundle_contracts(
    ...
    for entry in &bundle.contracts {
        let contract_path = Utf8Path::new(&entry.file);
        let contract = parse_harmony_facade_contract(entry.content.as_bytes(), contract_path)
            .with_context(|| format!("validating OHOS facade contract {} in {path}", entry.file))?;
        if entry.file != format!("{}.ohos-facade.json", contract.component) {
            bail!(
                "OHOS facade contract file does not match component {} in {path}",
                contract.component
            );
        }
        if !components.insert(contract.component.clone())
            || !namespaces.insert(contract.namespace.clone())
            || !prefixes.insert(contract.native_export_prefix.clone())
        {
            bail!("duplicate OHOS facade component, namespace, or native export prefix in {path}");
        }
    ...
    let owners = contracts
        ...
        .map(|contract| {
            (
                contract.component.as_str(),
                (
                    contract.namespace.as_str(),
                    contract.native_export_prefix.as_str(),
                ),
            )
        })
        .collect::<BTreeMap<_, _>>();
    for contract in &contracts {
        validate_harmony_facade_contract_owners(contract, &owners, path)?;
    }
    Ok(contracts)
}
...
fn parse_owned_bundle_type_defs(
    ...
    let sidecars = bundle
        ...
        .collect::<BTreeMap<_, _>>();
    if contracts.len() != sidecars.len() {
        bail!("required OHOS facade bundle must contain one type sidecar per contract in {path}");
    }
    let mut owned = Vec::new();
    for contract in contracts {
        let sidecar_file = format!("{}.ohos-extra-types.d.ts", contract.component);
        let entry = sidecars.get(sidecar_file.as_str()).with_context(|| {
            format!(
                "OHOS facade component {} is missing canonical sidecar {sidecar_file} in {path}",
                contract.component
            )
        })?;
        ...
        owned.push(OwnedFacadeTypeDefs::new(contract, defs)?);
    }
    let flattened = owned
        .iter()
...
struct InvocationFacadeInputs {
    _temp: tempfile::TempDir,
    work_dir: Utf8PathBuf,
}

impl InvocationFacadeInputs {
    fn new(
        target_dir: &Utf8Path,
        _package: &HostPackage,
        bundle: &HostFacadeBundle,
        ...
        std::fs::create_dir_all(target_dir)
            .with_context(|| format!("creating OHOS target directory {target_dir}"))?;
        let temp = tempfile::Builder::new()
            .prefix(".uniffi-ohos-facade-")
            .tempdir_in(target_dir)
            .with_context(|| {
                format!("creating invocation OHOS facade directory in {target_dir}")
            })?;
        let work_dir = Utf8PathBuf::from_path_buf(temp.path().to_path_buf()).map_err(|path| {
            anyhow::anyhow!("OHOS facade directory is not UTF-8: {}", path.display())
        })?;
        for entry in bundle.contracts.iter().chain(&bundle.type_sidecars) {
            ...
            let mut file = OpenOptions::new()
                ...
                .write(true)
                .open(&path)
                .with_context(|| format!("creating invocation facade input {path}"))?;
            file.write_all(entry.content.as_bytes())
                .with_context(|| format!("writing invocation facade input {path}"))?;
        }
        Ok(Self {
            _temp: temp,
            work_dir,
    ...
    F: FnMut(&Utf8Path) -> Result<()>,
{
    ...
    for (abi, libraries) in expected_raw {
        ...
        for (name, raw_digest) in libraries {
            ...
            let identity = runtime_so_identity(&bytes, abi, name)?;
...
fn emit_index_d_ts(
    ...
    let contracts = parse_bundle_contracts(bundle, Utf8Path::new("OHOS facade bundle"))?;
    let owned_defs =
        parse_owned_bundle_type_defs(bundle, Utf8Path::new("OHOS facade bundle"), &contracts)?;
    let defs = owned_defs
        .iter()
        .flat_map(|owned| owned.defs.iter().cloned())
        .collect::<Vec<_>>();
    let exports = FacadeExports::from_owned_type_defs_and_contracts(owned_defs, contracts)?;
...
impl From<&HarmonyFacadeContract> for FacadeComponentProjection {
    fn from(contract: &HarmonyFacadeContract) -> Self {
        Self {
            component: contract.component.clone(),
            namespace: contract.namespace.clone(),
            native_export_prefix: contract.native_export_prefix.clone(),
        }
    }
}
...
impl OwnedFacadeTypeDefs {
    fn new(contract: &HarmonyFacadeContract, defs: Vec<TypeDefLine>) -> Result<Self> {
        let projection = FacadeComponentProjection::from(contract);
...
impl FacadeExports {
    ...
    fn render_native_facade(&self, native_module: &str) -> String {
        ...
        out.push_str(
            &self
                .streams
                .render_native_facade(self.has_owned_projection()),
        );
        out
    }
    ...
    fn render_component_root_index(&self) -> String {
        ...
        for module in &modules {
            out.push_str("// eslint-disable-next-line @performance/no-use-any-import\n");
            out.push_str(&format!(
...
fn parse_harmony_facade_contract(bytes: &[u8], path: &Utf8Path) -> Result<HarmonyFacadeContract> {
    let contract: HarmonyFacadeContract = serde_json::from_slice(bytes)
        .with_context(|| format!("parsing OHOS facade contract in {path}"))?;
    validate_harmony_facade_contract_shape(&contract, path)?;
    Ok(contract)
}

fn validate_harmony_facade_contract_shape(
    contract: &HarmonyFacadeContract,
    path: &Utf8Path,
) -> Result<()> {
    ...
    {
        bail!("OHOS facade contract {path} has an invalid component name, namespace, or native export prefix");
    }
    for output in &contract.output_streams {
        validate_harmony_output_stream_contract_shape(
            output,
            &contract.native_export_prefix,
            path,
        )?;
    ...
    for input in &contract.input_streams {
        validate_harmony_input_stream_contract_shape(input, &contract.native_export_prefix, path)?;
    }
    Ok(())
}

fn validate_harmony_output_stream_contract_shape(
...
fn validate_harmony_input_stream_contract_shape(
...
fn validate_harmony_facade_contract_owners(
    contract: &HarmonyFacadeContract,
    components: &BTreeMap<&str, (&str, &str)>,
    path: &Utf8Path,
) -> Result<()> {
    for output in &contract.output_streams {
        output.item_type.validate_owners(components, path)?;
        output.error_type.validate_owners(components, path)?;
        for argument in &output.arguments {
            argument.r#type.validate_owners(components, path)?;
        }
    }
    for input in &contract.input_streams {
        input.item_type.validate_owners(components, path)?;
        input.error_type.validate_owners(components, path)?;
    }
    Ok(())
}
...
impl HarmonyTypeDescriptor {
    ...
    fn validate_owners(
        ...
        match self {
            Self::Named { owner, name } => {
                let (namespace, native_export_prefix) = components
                ...
                {
                    bail!(
                        "named type `{name}` in {path} has owner names that do not exactly match the host facade bundle"
                    );
                }
            }
            Self::Optional { inner } | Self::Sequence { inner } | Self::Set { inner } => {
                inner.validate_owners(components, path)?;
            }
            Self::Number
            | Self::Bigint
            | Self::Boolean
...
impl HarmonyStreamFacade {
    fn new(
        ...
        if owner_native_export_prefixes.len() != contracts.len() {
            bail!("duplicate Harmony stream facade contract component name");
        }
    ...
    fn render_native_facade(&self, export_owned_pull_classes: bool) -> String {
        let mut out = String::new();
        if !self.has_streams() {
            return out;
        }
...
#[cfg(test)]
#[path = "artifact_staging/ohos_characterization_tests.rs"]
mod tests;
