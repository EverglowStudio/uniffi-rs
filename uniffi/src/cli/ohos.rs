...
fn parse_harmony_facade_contract(value: &Value, path: &Utf8Path) -> Result<HarmonyFacadeContract> {
    let schema_version = value
        ...
        .with_context(|| {
            format!("OHOS facade contract schemaVersion in {path} must be an integer")
        })?;
    if schema_version != FACADE_CONTRACT_SCHEMA_VERSION {
        bail!(
            "unsupported OHOS facade contract schema in {path}; expected {FACADE_CONTRACT_SCHEMA_VERSION}, got {schema_version}"
        );
    }
