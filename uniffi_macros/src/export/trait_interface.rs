...
pub(crate) fn ffi_converter(trait_ident: &Ident, trait_kind: TraitKind) -> TokenStream {
    ...
    let try_lift = match trait_kind {
        TraitKind::RustOnly => {
            // The trait can only be implemented by Rust.
        ...
        TraitKind::ForeignOnly => {
            // The trait is only implemented by the foreign side.
            // All handles are foreign-generated.
            let trait_impl_ident = callback_interface::trait_impl_ident(&trait_name);
            quote! {
                fn try_lift(handle: Self::FfiType) -> ::uniffi::deps::anyhow::Result<::std::sync::Arc<Self>> {
                    ::std::result::Result::Ok(::std::sync::Arc::new(<#trait_impl_ident>::new(handle.as_raw())))
    ...
    let trait_kind_code = match trait_kind {
        ...
        TraitKind::ForeignOnly => quote! { ::uniffi::metadata::codes::TRAIT_KIND_FOREIGN_ONLY },
    };
