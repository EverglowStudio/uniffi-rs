...
| `support/**` | `artifacts build --javascript-support-dir <dir>` | Consumer-owned custom-type modules imported by generated facades |
...
### Consumer-owned custom-type support

Custom-type helper modules are supplied by the consumer, not generated by
UniFFI. Pass a real source directory to
`artifacts build --javascript-support-dir <dir>`. The directory is traversed
recursively; every entry must be a regular file or directory (symlinks and
other filesystem entries are rejected), and every file must decode as UTF-8
text. The validated tree is copied unchanged to the generated source root's
`support/` directory before any selected target is built. For example, with a
managed package this is `<package-dir>/src/ffi/support/`.

Configure relative custom-type imports for the shared component module, for
example `type { EmailAddress } from "../../support/email.js"` in a
`customTypes` entry. Generated shared modules retain that
`components/<namespace>/`-relative spelling. The ArkTS printer resolves the
same file and emits the package-root spelling
`./support/email.js` in `Index.ets`, so Harmony receives the same support tree
when its package is staged. A support source and its generated destination may
not overlap.

In managed layout, the support tree is part of the sibling staging directory
alongside generated source, native adapters, host crates, and Harmony outputs.
The complete managed root is published only after the build succeeds, so a
failed build leaves the previously published generation untouched. Support
has no manifest, schema, content hash, or identity metadata; the fixed managed
owner marker only protects the package root itself.
...
- Map values are real JavaScript `Map` instances at the public boundary in
  every engine (ECMAScript, Wasm, N-API/Electron, and ArkTS/Harmony), never
  plain objects or serialized entry arrays.
...
- Strict ArkTS exposes each declared error class directly from `Index.ets`.
  It does not re-export class constructors as component-object fields because
  ArkTS forbids constructor signatures and constructor type queries; callback
  error payloads use the public `ArkRecord` carrier.
...
`ready` and `init` share one one-shot promise and one `BackendSession`. The
public loader starts that promise with the deterministic module-relative URL
created as `new URL(<planned wasm path>, import.meta.url)`; argument-free
`init()` joins the same promise. The private `initWithGlue`
...
`wasm-bindgen-cli-support` engine in process; an external `wasm-bindgen`
executable, CLI installation, or source checkout is not a prerequisite.
