...
export class CallbackRegistry {
  ...
  async invokeAsync(callbackType: number, callbackId: number, methodId: number, invocationId: number, args: Array<ArkValue>): Promise<ArkValue> {
    ...
    try { return await entry.callback.invokeAsync(methodId, invocationId, args); }
...
export class Host {
  ...
  retainCallback(callbackType: number, callbackId: number): void { if (this.callbackRegistry === null) throw new UniffiError("UniffiCallbackMissing", "callback registry is not attached"); this.callbackRegistry.hostRetain(callbackType, callbackId); }
  releaseCallback(callbackType: number, callbackId: number): void { this.callbackRegistry?.hostRelease(callbackType, callbackId); }
...
interface ArkInputSlot { readonly pull: () => Promise<ArkStreamStep<ArkValue>>; readonly cancel: () => Promise<void>; readonly release: () => void; }
...
class ArkOutputStream<T> implements UniFfiStream<T> {
  ...
  async next(): Promise<ArkStreamStep<T>> {
    ...
    if (this.phase === "idle") {
      ...
      this.pending = true;
      try {
        this.handle = await this.spec.start();
    ...
    this.pending = true;
    try {
      const raw: ArkCallResult = await this.spec.next(this.handle);
...
export class BackendSession {
  ...
  constructor(backend: ArkBackend, host: Host = new Host()) { this.backend = backend; this.host = host; this.callbacks = new CallbackRegistry(); host.attachRegistry(this.callbacks, this); host.attachInputRegistry(this.inputRegistry, this); }
  private ensureOpen(): void { if (this.closed) throw new UniffiError("UniffiSessionClosed", "backend session is closed"); }
  ...
  invokeAsync(operationId: number, args: Array<ArkValue>): Promise<ArkValue> { this.ensureOpen(); return this.backend.invokeAsync(operationId, args).then((raw: ArkCallResult): ArkValue => __decodeResult(raw)); }
  ...
  releaseObject(handle: ArkValue): void { try { this.backend.releaseObject(handle); } catch (_) {} }
  ...
  createInputStream<T>(source: UniFfiInputStream<T>, lower: (value: T) => ArkValue): number { return this.inputRegistry.register(source, lower); }
  createInputView<T>(handle: ArkValue, lift: (value: ArkValue) => T): UniFfiInputStream<T> { return new ArkInputView<T>(this, handle, lift); }
  pullInputStream(handle: ArkValue): Promise<ArkStreamStep<ArkValue>> { return this.host.pullInputStream(handle); }
  cancelInputStream(handle: ArkValue): Promise<void> { return this.host.cancelInputStream(handle); }
  releaseInputStream(handle: ArkValue): void { this.host.releaseInputStream(handle); }
  releaseOutputStream(handle: ArkValue): void { this.backend.releaseOutputStream(handle); }
  createOutputStream<T>(spec: ArkOutputSpec<T>): UniFfiStream<T> { const stream: ArkOutputStream<T> = new ArkOutputStream<T>(this, spec); this.streams.push(stream as UniFfiStream<ArkValue>); return stream; }
  async close(): Promise<void> {
    if (this.closed) return;
    this.closed = true;
    const errors: Array<ArkValue> = [];
    for (const stream of this.streams) { try { await stream.cancel(); } catch (error) { if (error instanceof UniffiError) errors.push(error.data === null ? null : error.data); } }
    await this.inputRegistry.close();
    for (const object of this.objects.slice()) object.dispose();
    const result: Promise<void> | void | undefined = this.backend.close?.();
    if (result instanceof Promise) await result;
    if (errors.length > 0) throw new UniffiError("UniffiClose", "one or more resources failed to close", null, errors);
  }
}
...
fn render_callback_error_helper(
    ...
    out.push_str("  return new UniffiError(\"UniffiCallbackProtocol\", \"unknown callback error variant\");\n}\n");
    out
}
