...
//! adapter that satisfies the high-level TS API. The OHOS entrypoint shares
//! this generator, but writes the implementation-facing native facade and its
//! canonical sidecar separately from the public ArkTS package facade.
...
pub fn emit(
    ...
    //    it under the names the component's high-level common API expects.
    //    The adapter is intentionally thin: every non-trivial conversion
    //    lives in `uniffi_runtime_javascript`.
    ...
    // 3. Component flavor entry point. The namespace-only platform root
    //    re-exports this component API wired through the backend adapter.
...
fn render_index(ci: &uniffi_bindgen::ComponentInterface) -> String {
    ...
    format!(
        "// AUTOGENERATED by uniffi_bindgen_javascript (napi flavor).\n\
         //\n\
         // Node/N-API component entry for namespace `{namespace}`. Standard\n\
         // applications obtain this namespace from the namespace-only\n\
         // `node/index.ts` platform root, which wires this component API\n\
         // through the backend adapter on first import.\n\
         \n\
...
fn render_ohos_index(
    ...
    format!(
        "// AUTOGENERATED by uniffi_bindgen_javascript (harmony/ohos flavor).\n\
         //\n\
         // Harmony/OpenHarmony component entry for namespace `{namespace}`.\n\
         // Standard applications obtain this namespace from the namespace-only\n\
         // `Index.ets` platform root rather than importing this component file\n\
         // directly. The backend installs itself on first import and expects\n\
         // the consuming Harmony app to declare `{native_module}` in\n\
         // oh-package.json5.\n\
         \n\
...
fn render_ohos_stream_helpers(ci: &uniffi_bindgen::ComponentInterface) -> String {
    ...
    format!(
        "// AUTOGENERATED by uniffi_bindgen_javascript (harmony/ohos stream helpers).\n\
         //\n\
         // Each `fooStream` export is a source-level alias of the common\n\
         // `UniFfiStream` API. The OHOS packager projects it to the final\n\
         // HAR/HSP ArkTS Pull surface under the facade contract; this helper\n\
         // is not itself that final ArkTS implementation.\n\
         \n\
