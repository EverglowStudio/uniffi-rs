...
pub fn emit(
    ...
    let adapter = render_backend_adapter(ci, default_addon_path, AsyncCallbackAdapterMode::Promise);
...
fn render_backend_adapter(
    ...
    default_addon_path: Option<&str>,
    async_callback_mode: AsyncCallbackAdapterMode,
) -> String {
    ...
    let (async_callback_guard, async_infallible_callback, async_fallible_callback) =
        match async_callback_mode {
            AsyncCallbackAdapterMode::Promise => (
                "",
            ...
            AsyncCallbackAdapterMode::Direct => (
                "function __uniffiRequireSynchronousOhosCallbackResult(value: unknown): unknown {\n             if (\n                 value !== null &&\n                 typeof value === \"object\" &&\n                 typeof (value as { then?: unknown }).then === \"function\"\n             ) {\n                 throw new Error(\n                     \"Harmony async callback methods must return a synchronous value; Promise/thenable results are unsupported\",\n                 );\n             }\n             return value;\n         }\n",
                "const value = __uniffiRequireSynchronousOhosCallbackResult(fn(...liftedArgs));\n                                 return returnsCallback ? __uniffiStoreCallbackReturn(value) : __uniffiLowerShape(value);",
                "try {\n                                 const value = __uniffiRequireSynchronousOhosCallbackResult(fn(...liftedArgs));\n                                 return { ok: true, hasValue: value !== null && value !== undefined, value: returnsCallback ? __uniffiStoreCallbackReturn(value) : __uniffiLowerShape(value) };\n                             } catch (error) {\n                                 return { ok: false, hasValue: false, error: __uniffiCallbackErrorPayload(error, errorShape) };\n                             }",
            ),
        };
    format!(
             ...
             for (const [k, v] of Object.entries(obj as Record<string, unknown>)) {{\n\
                 if (typeof v === \"function\") {{\n\
                     out[k] = (...args: unknown[]) => {{\n\
                         ...
                         if (!errorShape) {{\n\
                             if (isAsync) {{\n\
                                 {async_infallible_callback}\n\
                             }}\n\
                             return __uniffiLowerShape(fn(...liftedArgs));\n\
                         }}\n\
                         if (isAsync) {{\n\
                             {async_fallible_callback}\n\
                         }}\n\
                         try {{\n\
...
fn render_ohos_backend_adapter(ci: &uniffi_bindgen::ComponentInterface) -> String {
    ...
    let mut adapter = render_backend_adapter(ci, None, AsyncCallbackAdapterMode::Direct)
        .replace(
            "// AUTOGENERATED by uniffi_bindgen_javascript (napi flavor).",
...
fn render_ohos_callback_type(name: &str, methods: &[&Method]) -> String {
    ...
    for method in methods {
        ...
        // ArkTS' TSFN return callback is synchronous.  Rust still exposes
        // the foreign trait method as `async`, but the OHOS bridge awaits the
        // one TSFN dispatch rather than a JavaScript Promise returned by it.
        out.push_str(&format!(
            "{}?: ({args}) => {ret}\n",
...
#[cfg(test)]
mod async_callback_adapter_tests {
    use super::*;
    use uniffi_meta::{MetadataGroup, NamespaceMetadata};

    fn empty_component() -> uniffi_bindgen::ComponentInterface {
        uniffi_bindgen::ComponentInterface::from_metadata(MetadataGroup {
            namespace: NamespaceMetadata {
                crate_name: "async_callback_adapter_fixture".into(),
                name: "async_callback_adapter_fixture".into(),
            },
            namespace_docstring: None,
            items: Default::default(),
        })
        .expect("empty adapter fixture metadata")
    }

    #[test]
    fn ohos_adapter_keeps_async_callback_markers_synchronous() {
        let ci = empty_component();
        let node = render_backend_adapter(&ci, None, AsyncCallbackAdapterMode::Promise);
        let ohos = render_ohos_backend_adapter(&ci);

        assert!(
            ohos.lines().all(|line| line.trim_end() == line),
            "OHOS adapter must not emit trailing whitespace:\n{ohos}"
        );
        assert!(
            node.contains("Promise.resolve(fn(...liftedArgs)).then"),
            "Node adapter must continue to Promise-wrap async callback markers:\n{node}"
        );
        assert!(
            !node.contains("hasValue: value !== null && value !== undefined"),
            "Node fallible async callback envelopes must remain unchanged:\n{node}"
        );
        assert!(
            ohos.contains("__uniffiRequireSynchronousOhosCallbackResult")
                && ohos.contains("Promise/thenable results are unsupported")
                && !ohos.contains("Promise.resolve(fn(...liftedArgs))"),
            "OHOS adapter must synchronously lower async markers and reject thenables:\n{ohos}"
        );
        assert!(
            ohos.contains("return { ok: true, hasValue:")
                && ohos.contains("return { ok: false, hasValue: false, error:"),
            "OHOS fallible async callbacks must build a synchronous result envelope:\n{ohos}"
        );
        assert!(
            ohos.contains("hasValue: value !== null && value !== undefined"),
            "OHOS fallible async callback envelopes must explicitly distinguish null success values:\n{ohos}"
        );
    }
}
